...
export default function Page() {
  ...
  const [stars, setStars] = useState<{ id: number; char: string; x: number; y: number }[]>([]);
  ...
  const [parallaxFactors, setParallaxFactors] = useState([-0.01, -0.005, 0.01, 0.015]);
  ...
  useEffect(() => {
    ...
    const regen = () => {
      ...
      setFieldSize(size);
      const count = Math.max(280, Math.floor(size * size * 0.00012));
    ...
    <main className="relative min-h-screen bg-[#07090c] text-zinc-200 font-mono overflow-hidden">
          ...
          >
            {parallaxFactors.map((factor, idx) => (
              <div key={idx} className="absolute inset-0 will-change-transform" style={{ transform: `translate3d(0, ${smoothY * factor * globalParallax}px, 0)` }}>
                <div
                  className="absolute inset-0 animate-[swirl_linear_infinite]"
                  style={{ animationDuration: `${(140 + idx * 30) / speedFactor}s` }}
                >
                  {stars.filter((_, i) => (i & 3) === idx).map((s) => (
                    <span
                      key={`l${idx}-${s.id}`}
                      style={{
                        position: "absolute",
      ...
      <div className="relative z-10 mx-auto max-w-6xl px-4 pb-20 pt-6 sm:px-6 lg:px-8">
        <div className="mb-4 flex items-center justify-between gap-4">
          ...
          <button onClick={() => setStarsOn((v) => !v)} className="group inline-flex items-center gap-2 rounded-md border border-zinc-700/70 bg-zinc-900/60 px-3 py-1.5 text-xs text-zinc-300 backdrop-blur transition hover:border-zinc-600 hover:text-zinc-100">Stars: {starsOn ? "On" : "Off"}</button>
        </div>
...
function ProjectCard({ title, tagline, img }: { title: string; tagline: string; img: string }) {
  return (
    <motion.article
