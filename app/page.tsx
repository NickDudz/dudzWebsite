...
export default function Page() {
  ...
  return (
    <main className="relative min-h-screen bg-[#07090c] text-zinc-200 font-mono overflow-hidden">
      ...
      {/* FPS Counter (responsive positioning) */}
      <FpsCounter
        show={showFpsCounter}
        getCurrentFps={galaxy.api?.getCurrentFps}
        getTargetFps={galaxy.api?.getTargetFps}
        getRenderStats={galaxy.api?.getRenderStats}
        className="fixed top-20 left-4 sm:top-16 sm:left-4 z-40 pointer-events-none"
      />

      <div className="relative z-[20] mx-auto max-w-6xl px-4 pb-20 pt-6 sm:px-6 lg:px-8 pointer-events-none">
        <div
          className={`mb-4 flex items-center justify-between gap-4 ${hudSidebar ? 'pr-[0px] sm:pr-[24rem]' : ''} ${
            galaxyOn ? 'pr-[200px] sm:pr-[240px]' : ''
          } pointer-events-none`}
        >
          <div className="flex items-center gap-4 pointer-events-auto">
            <div className="flex items-center gap-3">
              <a
                ...
                className={`${
                  panelsOn ? 'flex items-center gap-2 px-3 py-2' : 'p-2'
                } rounded-md border border-zinc-700/70 bg-zinc-900/60 text-xs text-zinc-300 backdrop-blur transition hover:border-zinc-600 hover:text-zinc-100 hover:bg-zinc-800/60`}
                title="Email"
              >
          ...
          {/* Expand Game Button - Moved to top row and centered*/}
          {panelsOn && (
            <div className="pointer-events-auto">
              <motion.button
                onClick={() => {
                  setPanelsOn(false)
                  setShowPanelsHint(true)
                  setTimeout(() => setShowPanelsHint(false), 5000)
                }}
        ...
        {/* Settings and GalaxyUI Buttons - Below email/github buttons */}
        <div className="flex justify-between items-center mb-4 pointer-events-none">
          <div className="flex items-center gap-3 pointer-events-auto">
            ...
            <SettingsDropdown
              ...
              onImportSave={() => false} // Not implemented yet
              onClearSave={() => galaxy.api?.clearSaveData?.() || false}
            />
          </div>
          
          <div className="flex items-center gap-3 pointer-events-auto">
            {/* GalaxyUI Trigger Button */}
            <button
              id="galaxy-ui-toggle"
            ...
            >
              <span 
                ...
                style={{
                  backgroundImage: 'linear-gradient(90deg, #3b82f6, #c084fc)',
        ...
        {/* Main Content Panels - Header and Project Grid */}
        {panelsOn && (
          <div className="pointer-events-auto">
      ...
      <div className="relative overflow-hidden">
        <div className="aspect-square w-full overflow-hidden">
          <img
            src={img}
            alt={title}
            className="h-full w-full object-cover object-center transition duration-300 group-hover:scale-[1.03] group-hover:opacity-95"
            loading="lazy"
          />
        </div>
        <div className="absolute bottom-2 left-2 right-2 flex items-end justify-between">
          ...
          <button className="rounded-sm border border-zinc-700/70 bg-zinc-900/60 px-2 py-1 text-[10px] font-medium text-zinc-200 opacity-90 hover:opacity-100">
            View
          </button>
        </div>
      </div>
